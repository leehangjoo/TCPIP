import threading
import socket
import tkinter as tk


class UiChatClient:
    # class 변수 / static 변수 : 모든 객체가 공유
    ip = 'localhost'
    port = 8080

    def __init__(self):
        self.conn_soc = None  # 서버와 연결된 소켓
        self.win = None
        self.chatCont = None
        self.myChat = None
        self.sendBtn = None
        self.allChat =''

    def conn(self): # 소켓 연결
        self.conn_soc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.conn_soc.connect((UiChatClient.ip, UiChatClient.port))

    def setWindow(self): # 인터페이스 설정
        self.win = tk.Tk()
        self.win.title('201744065 이항주')
        self.win.geometry('500x500+100+100')
        self.chatCont = tk.Label(self.win, width=60, height=40, text='dd')
        self.myChat = tk.Entry(self.win, width=50)
        self.sendBtn = tk.Button(self.win, width=20, text='전송')

        self.chatCont.grid(row=0, column=0, columnspan=2)
        self.myChat.grid(row=1, column=0, padx=10)
        self.sendBtn.grid(row=1, column=1)

        self.myChat.bind('<Return>', self.sendMsg)


    def sendMsg(self, e):  # 키보드 입력 받아 상대방에게 메시지 전송
        msg = self.myChat.get()
        self.myChat.delete(0, tk.END)
        self.myChat.config(text='')
        print(type(msg))
        msg = msg.encode(encoding='utf-8')
        print(self.conn_soc)
        self.conn_soc.sendall(msg)
        print('전송')

    def recvMsg(self):  # 상대방이 보낸 메시지 읽어서 화면에 출력
        print('읽기')
        while True:
            print('읽기')
            msg = self.conn_soc.recv(1024)
            print(msg)
            msg = msg.decode()+'\n'
            self.allChat += msg
            print(',:', self.allChat)

            self.chatCont.config(text=self.allChat)
            # if msg == '/stop':
            #     self.close()
            #     break

    def run(self): # 실행
        self.conn()
        self.setWindow()

        th2 = threading.Thread(target=self.recvMsg)
        th2.start()
        self.win.mainloop()

    def close(self): # 종료
        self.conn_soc.close()
        print('종료되었습니다')


def main():
    conn = UiChatClient()
    conn.run()


main()
