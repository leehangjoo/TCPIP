...
class Room:  # 채팅방 클래스.
    ...
    def addClient(self, c):  # 클라이언트 1명씩 전담하는 쓰레드
...
class ChatClient:  # 텔레마케터
    ...
    def readMsg(self):
        ...
        # cur = conn.cursor()
        # cur.execute('INSERT INTO id_save_data(name) VALUES(?)',msg)
        ...
        self.room.sendMsgAll(self.id + ' 퇴장')
...
class ChatServer:
    ...
    cur = conn.cursor()
    if One == 1:
        conn.execute('CREATE TABLE id_save_data(name TEXT)')
    def __init__(self):
